...
function sidebar($current) {
    ...
    if ($role == "admin") {
        ...
        echo '<a class="' . is_active("my_qr", $current) . '" href="../student/my_qr.php">🎫 <span class="text">My QR Code</span></a>';
...
function topbar() {
    ...
    // If the session was created before the profile photo was uploaded, read the latest photo from database.
    if ($profile_photo == "" && isset($conn) && isset($_SESSION["user_id"]) && $_SESSION["role"] != "admin") {
        $studentID = mysqli_real_escape_string($conn, $_SESSION["user_id"]);
        $photo_result = mysqli_query($conn, "SELECT profile_photo FROM student WHERE studentID = '$studentID' LIMIT 1");
    ...
    if ($profile_photo != "" && file_exists(__DIR__ . "/../" . $profile_photo)) {
        echo '<img class="avatar avatar-img" src="../' . clean($profile_photo) . '" alt="Profile">';
    } else {
